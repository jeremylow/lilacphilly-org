...
class BulletinHomePage(RoutablePageMixin, Page):
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        if month:
            bulletins = (
                bulletins_qs.filter(bulletinemail__bulletin_date__year=year)
                .filter(bulletinemail__bulletin_date__month=month)
                .order_by("-bulletinemail__bulletin_date")
            )
        else:
            bulletins = bulletins_qs.filter(
                bulletinemail__bulletin_date__month=datetime.datetime.now().month
            ).order_by("-bulletinemail__bulletin_date")
...
class BulletinEmail(OpenGraphMixin, Page):
    ...
    def save(self, *args, **kwargs):
        ...
        self.slug = "{}-{}".format(
            self.bulletin_date.strftime("%Y-%m-%d"), slugify(self.title)
        )
