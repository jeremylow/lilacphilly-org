...
class MemberCalendarHomePage(RoutablePageMixin, Page):
    ...
    def events(self, request, *args, **kwargs):
        ...
        return render(
            request, template_name, self.get_context(request, *args, **kwargs)
        )
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        if day:
            events = (
                events_qs.filter(membercalendarevent__event_date__year=year)
            ...
            events = events_qs.filter(
                membercalendarevent__event_date__year=year
            ).order_by("membercalendarevent__event_date")
        else:
            events = events_qs.filter(
        ...
        next_month = datetime.date(year, month, 1) + relativedelta.relativedelta(
            months=1
        )
        previous_month = datetime.date(year, month, 1) + relativedelta.relativedelta(
            months=-1
        )
        context = {
            "calendar": cal,
...
class MemberCalendarEvent(Page):
    ...
    def event_datetime(self, ev_date=None, ev_time=None):
        ...
        return datetime.datetime(
            ev_date.year, ev_date.month, ev_date.day, ev_time.hour, ev_time.minute
        )
    ...
    def event_start_dt(self):
        ...
        return self.event_datetime(
            ev_date=self.event_date, ev_time=self.event_start_time
        )
    ...
    def save(self, *args, **kwargs):
        ...
        self.slug = "{}-{}".format(
            self.event_date.strftime("%Y-%m-%d"), slugify(self.title)
        )
    ...
    def to_ical(self):
        ...
        event.add("uid", "{}@lilacphilly.org".format(self.slug))
        event.add(
            "dtstart", self.event_datetime(self.event_date, self.event_start_time)
        )
